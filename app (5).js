...
app.get('/library', authenticate, async (req, res) => {
    ...
    const query = `
        SELECT b.id, b.title, b.author, b.publisher, b.year_published, b.category, b.isbn 
        FROM user_libraries ul
        JOIN books b ON ul.book_id = b.id
        WHERE ul.user_id = ?`;
...
app.get('/allBooks', authenticate, (req, res) => {
    if (req.user.role !== 'admin') {
...
app.post('/deleteBook', authenticate, (req, res) => {
    if (req.user.role !== 'admin') {
        return res.status(403).send('Access Denied');
    }
    const { bookId } = req.body;

    // First, delete entries from user_libraries
    const deleteUserBookQuery = "DELETE FROM user_libraries WHERE book_id = ?";
    connection.query(deleteUserBookQuery, [bookId], (error, results) => {
        if (error) {
            console.error('Failed to delete user book links:', error);
            return res.status(500).send('Error deleting book from user libraries');
        }

        // If successful, proceed to delete the book from books table
        const deleteQuery = "DELETE FROM books WHERE id = ?";
        connection.query(deleteQuery, [bookId], (error, results) => {
            if (error) {
                console.error('Failed to delete book:', error);
                return res.status(500).send('Error deleting book');
            }
            res.send({ message: 'Book deleted successfully' });b
        });
    });
});

app.post('/removeBookFromLibrary', authenticate, (req, res) => {
    ...
    const deleteQuery = "DELETE FROM user_libraries WHERE user_id = ? AND book_id = ?";
    connection.query(deleteQuery, [req.user.userId, bookId], (error, results) => {
        if (error) {
            console.error('Failed to delete book from library:', error);
            return res.status(500).send('Error deleting book from user library');
        }
        if (results.affectedRows === 0) {
            return res.status(404).send('No book found with the given ID in user library');
        }
